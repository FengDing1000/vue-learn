let canvasFrame;
const createCanvas = {
    canvas: function(Scene,
        ShaderMaterial,
        PerspectiveCamera,
        BoxGeometry,
        Mesh,
        WebGLRenderer, TweenMax, that, ele) {
        ...
        function draw(props) {
            ...
            var geometry = new BoxGeometry(props.width, props.height, props.depth);
        ...
        function init() {
            scene = new Scene();
            camera = new PerspectiveCamera(100, sw / sh, 1, 10000);
            scene.add(camera);
            ...
            function listen(eventNames, callback) {
                for (var i = 0; i < eventNames.length; i++) {
                    if (eventNames[i] == "resize") {
                        window.addEventListener(eventNames[i], callback);
                    } else {
                        ele.addEventListener(eventNames[i], callback);
                    }
                }
            }
        ...
        function render(time) {
            ...
            // if (time < 800)
            window.headerCanvasFrame = requestAnimationFrame(render);
        }
        ...
        //console.log(THREE, TweenMax.TweenMax, planeTop, planeBottom);
        init();
    }
}
