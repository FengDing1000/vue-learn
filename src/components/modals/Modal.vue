<template>
<div v-show="modalShow" :transition="transition">
    <div class="modal" @click.self="clickMask">
        <div class="modal-dialog" :class="modalClass" ref="modalDialog">
...
export default {
    ...
    props: {
        ...
        cancelClass: {
            type: String,
            default: 'btn btn-primary btn-outline'
        },
        // 点击确定时关闭Modal
        // 默认为false，由父组件控制prop.modalShow来关闭
        closeWhenOK: {
            type: Boolean,
            default: false
        }
    ...
    watch: {
        modalShow(value) {
            ...
            // 在modal动画结束后再加上body滚动条
                ...
                if (!this.duration) {
                    this.duration = window.getComputedStyle(this.$refs.modalDialog)['transition-duration'].replace('s', '') * 1000;
                }
